...
def show_dist(bins, prob):
    import matplotlib.pyplot as plt
    a = [ np.random.choice(range(bins), p=prob.ravel()) for x in range(10000) ]
    plt.hist(a,bins+1)
    plt.show()
...
# Hyper-parameters
hidden_size   = 128  # hidden layer's size
seq_length    = 30   # number of steps to unroll
learning_rate = 1e-1
...
while True:
    ...
    if n % 2000 == 0:
        ...
        for t in range(sample_length):
            ...
            # if t==0 and n % 5000 ==0:
            #     show_dist(vocab_size,sample_output_softmax_val.ravel())
